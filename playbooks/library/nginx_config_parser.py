...
def create_readable_nginx_config(result):
    ...
    def process_parsed_list(parsed_list, config_list, level=0):
        ...
        for directive in parsed_list:
            ...
            if 'block' in directive and directive['block']:
                ...
                config[key] = process_parsed_list(directive['block'],
                                                  config_list, level + 1)
                    ...
                    for include_id in directive['includes']:
                        if include_id < len(config_list):
                            ...
                            file_content = process_parsed_list(
                                included_file['parsed'], config_list,
                                level + 1)
    ...
    readable_config = process_parsed_list(main_config['parsed'],
                                          result['config'])
...
DIRECTIVES = {
    ...
    'location': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
                 NGX_CONF_1MORE | NGX_CONF_BLOCK],
    'upstream': [NGX_HTTP_MAIN_CONF | NGX_CONF_TAKE1 | NGX_CONF_BLOCK],
    'include': [NGX_MAIN_CONF | NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                NGX_HTTP_LOC_CONF | NGX_CONF_TAKE1],
    'listen': [NGX_HTTP_SRV_CONF | NGX_CONF_1MORE],
    'server_name': [NGX_HTTP_SRV_CONF | NGX_CONF_1MORE],
    'root': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
             NGX_CONF_TAKE1],
    'index': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
              NGX_CONF_1MORE],
    'error_page': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                   NGX_HTTP_LOC_CONF | NGX_CONF_2MORE],
    'access_log': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                   NGX_HTTP_LOC_CONF | NGX_CONF_1MORE],
    'error_log': [NGX_MAIN_CONF | NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                  NGX_HTTP_LOC_CONF | NGX_CONF_1MORE],
    'worker_processes': [NGX_MAIN_CONF | NGX_CONF_TAKE1],
    'worker_connections': [NGX_EVENT_CONF | NGX_CONF_TAKE1],
    'keepalive_timeout': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF |
                          NGX_HTTP_LOC_CONF | NGX_CONF_TAKE12],
    'gzip': [NGX_HTTP_MAIN_CONF | NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF |
             NGX_CONF_FLAG],
    ...
    'if': [NGX_HTTP_SRV_CONF | NGX_HTTP_LOC_CONF | NGX_CONF_1MORE |
           NGX_CONF_BLOCK],
}
...
def analyze(fname, stmt, term, ctx=(), strict=False, check_ctx=True,
    ...
    for mask in reversed(masks):
        ...
        if ((mask >> n_args & 1 and n_args <= 7) or  # NOARGS to TAKE7
            ...
            reason = ('invalid value "%s" in "%%s" directive, '
...
def main():
    module_args = dict(
        ...
        ignore_directives=dict(type='list', elements='str', required=False,
                               default=[]),
    ...
    try:
        ...
        module.fail_json(msg="Failed to parse nginx configuration: %s" %
                         str(e), **result)
