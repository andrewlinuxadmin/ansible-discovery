...
class ApacheConfigLexer(object):
    ...
    def tokenize(self, text):
        ...
        for line_num, line in enumerate(lines, 1):
            ...
            if (lower_line.startswith('include ') or
                    lower_line.startswith('includeoptional ')):
                match = re.match(r'^(include(?:optional)?)\s+(.+)$',
                                 line, re.IGNORECASE)
                if match:
...
class ApacheConfigLoader(object):
    ...
    def load(self, config_file):
        ...
        config_root = self._options.get(
            'configroot', os.path.dirname(config_file))
    ...
    def _load_file_recursive(self, filepath):
        ...
        if (filepath in self._includes and 
                not self._options.get('includeagain')):
            return {}
        ...
        try:
            ...
            raise ConfigFileReadError('File %s can\'t be open: %s'
                                      % (filepath, ex))
        ...
        for configdir in configpath:
            ...
            if options.get('includeglob') and ('*' in filepath or '?' in filepath):
                ...
                if options.get('includedirectories'):
                    contents = {}
                    try:
                        for include_file in sorted(
                                self._reader.listdir(full_filepath)):
                            if include_file.endswith('.conf'):
                                include_path = os.path.join(
                                    full_filepath, include_file)
...
def main():
    module = AnsibleModule(
        argument_spec=dict(
            ...
            output_format=dict(type='str', choices=['dict', 'json'],
                               default='dict'),
