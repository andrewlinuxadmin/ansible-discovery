...
class MongoDBQueryTester:
    ...
    def __init__(self, api_host: str = "localhost", api_port: int = 8000, verbose: bool = False):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if status in ['ok', 'healthy']:
                    ...
                    self.log(f"❌ API returned unexpected status: {data.get('status')}", "ERROR")
    ...
    def test_query(self, name: str, query: List[Dict]) -> QueryResult:
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                # Only consider success if we have actual data
                if result.document_count > 0:
                    result.success = True
                    self.log(f"✅ {name}: {result.document_count} docs, {execution_time:.2f}s [{category}]", "SUCCESS")
                else:
                    # Query executed successfully but returned no data - this is a warning, not a failure
                    result.success = True  # Technical success - query executed
                    result.error_message = "Query executed successfully but returned no data"
                    self.log(f"⚠️  {name}: No data returned, {execution_time:.2f}s [{category}]", "WARNING")
    ...
    def generate_report(self, results: List[QueryResult]) -> str:
        ...
        no_data = [r for r in failed if r.error_message and "returned no data" in r.error_message]
        actual_failed = [r for r in failed if not (r.error_message and "returned no data" in r.error_message)]
        ...
        report.append(f"   • ⚠️  No Data: {len(no_data)} ({len(no_data)/len(results)*100:.1f}%)")
        report.append(f"   • ❌ Failed: {len(actual_failed)} ({len(actual_failed)/len(results)*100:.1f}%)")
        ...
        if no_data:
            report.append("⚠️  QUERIES WITH NO DATA:")
            report.append("-" * 40)
            for result in no_data:
                report.append(f"   • {result.name}")
                report.append(f"     └─ Query executed successfully but no data found, {result.execution_time:.2f}s")
            report.append("")
        
        if actual_failed:
            report.append("❌ FAILED QUERIES:")
            report.append("-" * 40)
            for result in actual_failed:
                report.append(f"   • {result.name}")
    ...
    def generate_json_report(self, results: List[QueryResult]) -> Dict[str, Any]:
        ...
        for result in results:
            if result.category not in categories:
                categories[result.category] = {"total": 0, "successful": 0, "no_data": 0, "failed": 0, "documents": 0}
            categories[result.category]["total"] += 1
            if result.success:
                categories[result.category]["successful"] += 1
                categories[result.category]["documents"] += result.document_count
            elif result.error_message and "returned no data" in result.error_message:
                categories[result.category]["no_data"] += 1
            else:
...
Examples:
  ...
  python test_all_queries.py --html custom-queries.html
    ...
    parser.add_argument(
        ...
        type=int,
        default=8000,
        help='MongoDB proxy API port (default: 8000)'
    )
    parser.add_argument(
        '--html',
    ...
    # Exit with error code only if queries actually failed (not just empty results)
    failed_count = len([r for r in results if not r.success])
    no_data_count = len([r for r in results if r.success and r.document_count == 0])
    successful_count = len([r for r in results if r.success and r.document_count > 0])
    
    if failed_count > 0:
        print(f"\n❌ {failed_count} queries failed")
        sys.exit(1)
    elif no_data_count > 0 and successful_count > 0:
        print(f"\n🎉 {successful_count} queries successful, ⚠️  {no_data_count} returned no data")
        sys.exit(0)
    elif no_data_count > 0:
        print(f"\n⚠️  All {no_data_count} queries returned no data (but executed successfully)")
        sys.exit(0)
    else:
